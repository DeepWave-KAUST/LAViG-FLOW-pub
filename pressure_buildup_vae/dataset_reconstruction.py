###############################################################################
# Pressure Build-Up dataset reconstruction helpers (2025)
#
...
class ReconstructionAssets:
    ...
    porosity_min: float
    porosity_max: float
    pressure_global_min: float
    pressure_global_max: float
...
SPLIT_SPECS: Dict[str, SplitSpec] = {
    "train": SplitSpec(
        name="train",
        target_path=Path("dP_data/train_dP_target.pt"),
        inputs_path=Path("dP_data/training_dataset_pressure_buildup/dP_train_inputs.pt"),
        stats_path=Path("dP_data/training_dataset_pressure_buildup/dP_train_min_max.pt"),
        metrics_filename="vae_train_dP_average_metrics.txt",
        mosaic_prefix="train",
    ),
    "validation": SplitSpec(
        name="validation",
        target_path=Path("dP_data/val_dP_target.pt"),
        inputs_path=Path("dP_data/validation_dataset_pressure_buildup/dP_val_inputs.pt"),
        stats_path=Path("dP_data/validation_dataset_pressure_buildup/dP_val_min_max.pt"),
        metrics_filename="vae_validation_dP_average_metrics.txt",
        mosaic_prefix="val",
    ),
    "test": SplitSpec(
        name="test",
        target_path=Path("dP_data/test_dP_target.pt"),
        inputs_path=Path("dP_data/test_dataset_pressure_buildup/dP_test_inputs.pt"),
        stats_path=Path("dP_data/test_dataset_pressure_buildup/dP_test_min_max.pt"),
        metrics_filename="vae_test_dP_average_metrics.txt",
        mosaic_prefix="test",
    ),
}
...
# --------------------------------------------------
def build_model(config: Dict, checkpoint: Path, device: torch.device) -> VAE:
    """Instantiate the VAE architecture and load weights from checkpoint."""
    ...
    model = VAE(im_channels=im_channels, autoencoder_model_config=auto_cfg)
...
def load_split_assets(spec: SplitSpec) -> ReconstructionAssets:
    ...
    pressure_global_min = float(per_sample_min.min().item())
    pressure_global_max = float(per_sample_max.max().item())
    ...
    return ReconstructionAssets(
        ...
        pressure_global_min=pressure_global_min,
        pressure_global_max=pressure_global_max,
...
# --------------------------------------------------
# Math: map [-1,1] sample back to data range
# --------------------------------------------------
def denormalize_sample(normalized: np.ndarray, min_value: float, max_value: float) -> np.ndarray:
    """Bring a [-1, 1] normalised sample back to its original range."""
    if np.isclose(max_value, min_value):
        return np.full_like(normalized, fill_value=min_value)
    return (normalized + 1.0) * 0.5 * (max_value - min_value) + min_value


# --------------------------------------------------
# Math: map [-1,1] porosity back to data range
# --------------------------------------------------
def denormalize_porosity(normalized: np.ndarray, min_value: float, max_value: float) -> np.ndarray:
    """Restore porosity values from [-1, 1] to physical units."""
    if np.isclose(max_value, min_value):
        return np.full_like(normalized, fill_value=min_value)
    return (normalized + 1.0) * 0.5 * (max_value - min_value) + min_value
...
def save_colorbar_grid(
    ...
    rows = [
        ("Original (bar)", original_ranges, cmap_targets),
        ("Reconstructed (bar)", reconstructed_ranges, cmap_targets),
        ("Error (bar)", error_ranges, cmap_error),
    ]
...
def evaluate_split(
    ...
    Dict[str, float],
    List[Tuple[int, np.ndarray, np.ndarray, np.ndarray, int, float, float, float]],
    List[Tuple[int, List[Tuple[int, np.ndarray, np.ndarray, np.ndarray, int, float, float, float]]]],
    List[int],
]:
    ...
    mosaic_payload: List[Tuple[int, np.ndarray, np.ndarray, np.ndarray, int, float, float, float]] = []
    clip_storage: Dict[int, List[Optional[Tuple[int, np.ndarray, np.ndarray, np.ndarray, int, float, float, float]]]] = {
    ...
    model.eval()
    with torch.no_grad():
        for sample_idx in tqdm(eval_indices, desc="Evaluating samples", leave=False):
            try:
                data = assets.targets[sample_idx : sample_idx + 1].to(device)
                cond = assets.conditions[sample_idx : sample_idx + 1].to(device)

                _, encoder_output = model.encode(data)
                mean, _ = torch.chunk(encoder_output, 2, dim=1)
                reconstructed = model.decode(mean)

                original = data[0].detach().cpu().numpy()
                condition = cond[0].detach().cpu().numpy()
                recon = reconstructed[0].detach().cpu().numpy()

                sample_min = float(assets.per_sample_min[sample_idx].item())
                sample_max = float(assets.per_sample_max[sample_idx].item())

                denorm_condition = denormalize_porosity(condition, assets.porosity_min, assets.porosity_max)
                denorm_original = denormalize_sample(original, sample_min, sample_max)
                denorm_reconstructed = denormalize_sample(recon, sample_min, sample_max)

                mask = denorm_condition[0] != 0
                thickness = int(mask[:, 0].sum())
                if thickness <= 0:
                    raise ValueError("Encountered porosity mask with zero thickness.")

                masked_original = denorm_original[0][mask].reshape(thickness, -1)
                masked_reconstructed = denorm_reconstructed[0][mask].reshape(thickness, -1)
                ...
                mae = mean_absolute_error(masked_original[nonzero_mask], masked_reconstructed[nonzero_mask])
                mse = mean_squared_error(masked_original[nonzero_mask], masked_reconstructed[nonzero_mask])
                rmse = math.sqrt(mse)
                smape = np.mean(
                    2.0 * np.abs(masked_original - masked_reconstructed)
                    / (np.abs(masked_original) + np.abs(masked_reconstructed) + 1e-1)
                ) * 100.0
                ...
                orig_vmin = float(masked_original.min())
                orig_vmax = float(masked_original.max())
                err_vmax = float(masked_error.max()) if np.isfinite(masked_error).any() else 0.0
                if np.isclose(orig_vmin, orig_vmax):
                    orig_vmax = orig_vmin + 1e-6
                if err_vmax <= 0:
                    err_vmax = 1e-6

                if sample_idx in sample_set:
                    mosaic_payload.append(
                        (
                            sample_idx,
                            masked_original,
                ...
                if clip_set:
                    clip_idx = sample_idx // frames_per_clip
                    if clip_idx in clip_storage:
                        frame_idx = sample_idx % frames_per_clip
                        clip_storage[clip_idx][frame_idx] = (
                            sample_idx,
                            masked_original,
                            masked_reconstructed,
                            masked_error,
    ...
    evaluated_samples = len(eval_indices) - len(skipped_samples)
    pressure_range = assets.pressure_global_max - assets.pressure_global_min
    pressure_range = pressure_range if pressure_range > 0 else eps

    metrics = {
        "evaluated_samples": evaluated_samples,
        "average_mae": float(np.mean(mae_vals)) if mae_vals else float("nan"),
        "average_mse": float(np.mean(mse_vals)) if mse_vals else float("nan"),
        "average_rmse": float(np.mean(rmse_vals)) if rmse_vals else float("nan"),
        "average_r2": float(np.mean(r2_vals)) if r2_vals else float("nan"),
        "average_smape": float(np.mean(smape_vals)) if smape_vals else float("nan"),
        "average_ssim": float(np.mean(ssim_vals)) if ssim_vals else float("nan"),
        "average_nmae": float(np.mean(mae_vals) / pressure_range) if mae_vals else float("nan"),
        "average_nmse": float(np.mean(mse_vals) / (pressure_range ** 2)) if mse_vals else float("nan"),
        "average_nrmse": float(np.mean(rmse_vals) / pressure_range) if rmse_vals else float("nan"),
    }

    clip_payload: List[
        Tuple[int, List[Tuple[int, np.ndarray, np.ndarray, np.ndarray, int, float, float, float]]]
    ] = []
    for clip_idx, frames in clip_storage.items():
        valid = [frame for frame in frames if frame is not None]
        if len(valid) == frames_per_clip:
            clip_payload.append((clip_idx, valid))
...
def write_metrics(metrics: Dict[str, float], output_path: Path) -> None:
    lines = [
        "====== AVERAGE METRICS ACROSS SELECTED SAMPLES ======",
        f"Evaluated samples: {metrics['evaluated_samples']}",
        f"Average MAE: {metrics['average_mae']:.6f}",
        f"Average NMAE: {metrics['average_nmae']:.6f}",
        f"Average MSE: {metrics['average_mse']:.6f}",
        f"Average NMSE: {metrics['average_nmse']:.6f}",
        f"Average RMSE: {metrics['average_rmse']:.6f}",
        f"Average NRMSE: {metrics['average_nrmse']:.6f}",
        f"Average R²: {metrics['average_r2']:.6f}",
        f"Average sMAPE: {metrics['average_smape']:.2f}%",
        f"Average SSIM: {metrics['average_ssim']:.6f}",
    ]
...
def render_mosaic_pages(
    samples: List[Tuple[int, np.ndarray, np.ndarray, np.ndarray, int, float, float, float]],
    output_dir: Path,
    prefix: str,
    cols_per_page: int,
    ...
    """Render 3-row mosaics (original, reconstruction, absolute error) with per-sample colour scales."""
    if not samples:
        return
    ...
    for page_idx in range(pages):
        ...
        X, Y = build_xy_grid(max_thickness, width)
        ...
        for col_idx, (_, original, reconstructed, error, thickness, vmin, vmax, err_vmax) in enumerate(chunk):
            ...
            orig_ranges.append((vmin, vmax))
            recon_ranges.append((vmin, vmax))
            error_max = max(err_vmax, 1e-6)
            error_ranges.append((0.0, error_max))
...
def render_clip_mosaics(
    ...
    for clip_idx, frames in clips:
        ...
        for col_idx, (_, original, reconstructed, error, thickness, vmin, vmax, err_vmax) in enumerate(frames_sorted):
            orig_img = pad_to(original[:thickness, :], max_thickness, width)
            recon_img = pad_to(reconstructed[:thickness, :], max_thickness, width)
            err_img = pad_to(error[:thickness, :], max_thickness, width)
...
def parse_common_args(default_split: str, argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate VAE reconstructions for the pressure build-up dataset."
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("dP.yaml"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Output directory for metrics and mosaics (defaults to results/reconstructions/<split>).",
    )
    parser.add_argument(
        "--split",
    ...
    parser.add_argument(
        ...
        type=int,
        default=17,
        help="Number of samples (or clips) to include across mosaic pages (default: 17).",
    )
    parser.add_argument(
        "--mosaic-cols",
        type=int,
        default=17,
        help="Number of columns per mosaic page for sample mode (default: 17).",
    )
    parser.add_argument(
        "--mosaic-mode",
        choices=("clip", "sample"),
...
def run_cli(default_split: str, argv: Optional[Sequence[str]] = None) -> None:
    ...
    output_dir = args.output_dir if args.output_dir is not None else Path("results") / "reconstructions" / spec.name
    output_dir = make_absolute(output_dir if isinstance(output_dir, Path) else Path(output_dir))
    ...
    print(
        ...
        f"Average NMAE: {metrics['average_nmae']:.4f} | "
