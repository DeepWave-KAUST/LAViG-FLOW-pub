###############################################################################
# LPIPS perceptual metric helpers (2025)
#
...
# Notes:
#   Taken from:
#     - https://github.com/explainingai-code/StableDiffusion-PyTorch/blob/main/models/lpips.py
#     - https://github.com/richzhang/PerceptualSimilarity/blob/master/lpips/lpips.py
#   Pre-trained VGG-16 weights (v0.1) by Richard Zhang et al.:
...
from paths import GAS_SATURATION_VQVAE_ROOT

accelerator = Accelerator()

# Script sources:
#   StableDiffusion-PyTorch LPIPS implementation (explainingai-code)
#   Richard Zhang's PerceptualSimilarity project (official LPIPS release)

# --------------------------------------------------
# Reduction: spatial mean over HxW
# --------------------------------------------------
...
# --------------------------------------------------
# Backbone: VGG-16 feature extractor slices
# --------------------------------------------------
...
# --------------------------------------------------
# Metric: Learned Perceptual Image Patch Similarity
# --------------------------------------------------
class LPIPS(nn.Module):
    def __init__(self, net='vgg', version='0.1', use_dropout=True):
        ...
        self.lins = nn.ModuleList(self.lins).to(accelerator.device)
        ########################
        
        # Load the weights of trained LPIPS model
        model_path = GAS_SATURATION_VQVAE_ROOT / "trained_models/vgg.pth"
        print(f'Loading model from: {model_path}')
        self.load_state_dict(torch.load(model_path, map_location=accelerator.device), strict=False)
        ########################
...
# --------------------------------------------------
# Normalisation: ImageNet shift/scale buffers
# --------------------------------------------------
...
# --------------------------------------------------
# Linear: 1x1 projection with optional dropout
# --------------------------------------------------
