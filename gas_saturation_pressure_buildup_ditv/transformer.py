#!/usr/bin/env python3
###############################################################################
# DiTV Transformer (Video Diffusion inspired) (2025)
#
# Author: Vittoria De Pellegrini
# Affiliation: PhD Student, King Abdullah University of Science and Technology
#
# Description:
#   Joint CO₂ gas + pressure DiTV backbone adapted from the Video Diffusion
#   reference implementation (https://github.com/explainingai-code/VideoGeneration-PyTorch),
#   extended with adaptive LayerNorm parameters for diffusion conditioning.
###############################################################################
...
class DITVideo(nn.Module):
    ...
    and temporal encoder layers, each of which are DiT blocks. This is Variant I. 
    """
    def __init__(self, frame_height, frame_width, im_channels, num_frames, config):
        super().__init__()
        ...
        # Final Linear Layer
        self.output_channels = self.im_channels * 2
        self.proj_out = nn.Linear(
            self.hidden_size,
            self.patch_height * self.patch_width * self.output_channels,
        )
    ...
    def forward(self, x, t, num_images=0):
        ...
        B, F, C, H, W = x.shape # C is the latent channels because we are working in the latent space 
        ...
        out = out.reshape((B, F, self.output_channels, H, W))
        return out