...
#
# Description: ufno/train_UFNO_dp.py
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train U-FNO pressure buildup model")
...
def main():
    ...
    run_name = f"ufno_dp_{time.strftime('%Y%m%d-%H%M%S')}"
    run_config = {
        "model": "ufno",
        "target": "dP",
    ...
    for ep in range(1, epochs + 1):
        ...
        if accelerator.is_main_process and ep % save_every == 0:
            path = f"{save_dir}/dP_UFNO_{ep}ep_{width}width_{mode1}m1_{mode2}m2_{train_a.shape[0]}train_{lr_:.2e}lr"
