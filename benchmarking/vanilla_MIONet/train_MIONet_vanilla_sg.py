...
#
# Description: vanilla_MIONet/train_MIONet_vanilla_sg.py
...
from model import Encoder, MIONetCartesianProd
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train vanilla MIONet gas saturation model")
...
def load_split(data_dir, split, n_frames, n_samples=None):
    if split == "train":
        input_path = os.path.join(
            data_dir,
            "co2_data",
            "training_dataset_gas_saturation",
            "sg_train_inputs.pt",
        )
        ...
        input_path = os.path.join(
            data_dir,
            "co2_data",
            "validation_dataset_gas_saturation",
            "sg_val_inputs.pt",
        )
        output_path = os.path.join(
            data_dir,
...
def main():
    ...
    net = MIONetCartesianProd(
        layer_sizes_branch1=[y_train.shape[1] * 3, Encoder()],
    ...
    pre_mask_train = np.isclose(y_train.reshape(y_train.shape[0], n_frames, 96, 200)[:, -1, :, 0], 0.0)
    pre_mask_test = np.isclose(y_test.reshape(y_test.shape[0], n_frames, 96, 200)[:, -1, :, 0], 0.0)
    ...
    def mae(y_true, y_pred):
        ...
        for i in range(size):
            ...
            y_true_i = y_true[i][:, mask, :]
            y_pred_i = y_pred[i][:, mask, :]
            err += np.mean(np.abs(y_true_i.flatten() - y_pred_i.flatten()))
        return err / size

    os.makedirs(save_dir, exist_ok=True)

    run_name = f"vanilla_mionet_sg_{time.strftime('%Y%m%d-%H%M%S')}"
    run_config = {
        "model": "vanilla-mionet",
        "target": "sg",
        "n_frames": n_frames,
        "iterations": int(cfg.get("iterations", cfg.get("epochs", 112500))),
    ...
    model.compile(
        ...
        decay=("step", int(cfg.get("decay_step", 4500)), float(cfg.get("decay_gamma", 0.9))),
        metrics=[rsquare_plume_together, mae],
    )

    checkpoint_prefix = os.path.join(save_dir, "MIONet_vanilla_SG")
    checker = dde.callbacks.ModelCheckpoint(
        checkpoint_prefix,
        save_better_only=bool(cfg.get("save_better_only", False)),
        period=int(cfg.get("save_every", 2250)),
        monitor="test loss",
    )
    ...
    losshistory, train_state = model.train(
        ...
        display_every=int(cfg.get("display_every", 750)),
        init_test=True,
        callbacks=callbacks,
    )
