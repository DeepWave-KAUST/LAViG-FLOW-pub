...
class Model:
    ...
    def _compile_pytorch(self, lr, loss_fn, decay, loss_weights):
        ...
        def outputs_losses(training, inputs, targets,train_indices,istrain, losses_fn):
            self.net.train(mode=training)
            if isinstance(inputs, tuple):
                inputs = tuple(
            ...
            losses = losses_fn(targets, outputs_, train_indices,istrain, loss_fn, inputs, self)
            if not isinstance(losses, list):
                losses = [losses]
        ...
        def outputs_losses_train(inputs, targets,train_indices,istrain):
            return outputs_losses(True, inputs, targets,train_indices,istrain, self.data.losses_train)

        def outputs_losses_test(inputs, targets,train_indices,istrain):
            return outputs_losses(False, inputs, targets,train_indices,istrain, self.data.losses_test)
        ...
        def train_step(inputs, targets,train_indices,istrain):
            def closure():
                losses = outputs_losses_train(inputs, targets,train_indices,istrain)[1]
    ...
    def _outputs_losses(self, training, inputs, targets, auxiliary_vars,train_indices=None):
        if training:
        ...
        if backend_name == "tensorflow":
            ...
            outs = outputs_losses(inputs, targets,train_indices,training)
    ...
    def _train_step(self, inputs, targets, train_indices,istrain,auxiliary_vars):
        if backend_name == "tensorflow.compat.v1":
            ...
            # TODO: auxiliary_vars
            self.train_step(inputs, targets,train_indices,istrain)
        elif backend_name == "jax":
            # TODO: auxiliary_vars
    ...
    def train(
        ...
        iterations=None,
        batch_size=None,
        timestep_batch_size=None,
        training_time_size=None,
        display_every=1000,
	...
	init_test = True
        ...
        self.training_time_size = training_time_size
        ...
        self.stop_training = False
        self.train_state.set_data_train(*self.data.train_next_batch(self.batch_size,self.timestep_batch_size,self.training_time_size))
    ...
    def _train_sgd(self, iterations, display_every):
        for i in range(iterations):
            ...
            self.train_state.set_data_train(
                *self.data.train_next_batch(self.batch_size,self.timestep_batch_size,self.training_time_size)
            )
            self._train_step(
                self.train_state.X_train,
                self.train_state.y_train,
                self.train_state.train_indices,
                True,
    ...
    def _test(self):
        #print(self.train_state.y_test.shape)
        (
        ...
        # self.train_state.y_pred_test, self.train_state.loss_test = self._outputs_losses(
        #         False,
        #         (self.train_state.X_test[0][i*4:(i+1)*4],self.train_state.X_test[1][i*4:(i+1)*4],self.train_state.X_test[2]),
        #         self.train_state.y_test[i*4:(i+1)*4],
        #         self.train_state.test_aux_vars,
        #     )
        # self.train_state.y_pred_test = self.train_state.y_pred_test.reshape(4,24,96,200)
        self.train_state.loss_test = 0
        batch_size = 4
        timestep_batch_size = self.timestep_batch_size
        x_loc = self.train_state.X_test[2]
        if x_loc.ndim == 1:
            x_loc = x_loc[:, None]
        if x_loc.ndim != 2:
            raise ValueError(
                "Expected X_test[2] to have shape (N, dim). "
                f"Got shape {x_loc.shape}."
            )
        test_size = self.train_state.y_test.shape[0]
        num_time_batches = int(np.ceil(self.training_time_size / timestep_batch_size))
        expected_grid = self.training_time_size * 96 * 200
        for i in range(int(test_size / batch_size)):
            for j_start in range(0, self.training_time_size, timestep_batch_size):
                j_end = min(j_start + timestep_batch_size, self.training_time_size)
                current_timestep_batch = j_end - j_start
                if x_loc.shape[0] == self.training_time_size:
                    x_loc_batch = x_loc[j_start:j_end]
                elif x_loc.shape[0] == expected_grid:
                    x_loc_batch = x_loc.reshape(self.training_time_size, 96 * 200, 3)[
                        j_start:j_end, :
                    ].reshape(current_timestep_batch * 96 * 200, 3)
                else:
                    raise ValueError(
                        "training_time_size does not match X_test[2]. "
                        f"training_time_size={self.training_time_size}, X_test[2].shape[0]={x_loc.shape[0]}."
                    )
                y_pred_test_timestepbatch, loss_test_batch = self._outputs_losses(
                    False,
                    (self.train_state.X_test[0][i*batch_size:(i+1)*batch_size],
                    self.train_state.X_test[1][i*batch_size:(i+1)*batch_size],
                    x_loc_batch),
                    self.train_state.y_test.reshape(test_size, self.training_time_size, 96 * 200)[
                        i * batch_size:(i + 1) * batch_size, :
                    ][:, j_start:j_end].reshape(
                        batch_size, current_timestep_batch * 96 * 200
                    ),
                    self.train_state.test_aux_vars,
                    range(i*batch_size,(i+1)*batch_size),
                )
                y_pred_test_timestepbatch = y_pred_test_timestepbatch.reshape(
                    batch_size, current_timestep_batch * 96 * 200
                )
                if j_start == 0:
                    y_pred_test_batch = y_pred_test_timestepbatch
                else:
                    y_pred_test_batch = np.concatenate((y_pred_test_batch, y_pred_test_timestepbatch),1)
                self.train_state.loss_test +=loss_test_batch
            if i == 0:
                self.train_state.y_pred_test = y_pred_test_batch
            else:
                self.train_state.y_pred_test = np.concatenate(
                    (self.train_state.y_pred_test, y_pred_test_batch), 0
                )
        self.train_state.loss_test /= int(test_size / batch_size) * num_time_batches
        self.train_state.y_pred_test = self.train_state.y_pred_test.reshape(
            test_size, self.training_time_size * 96 * 200
        )
...
class TrainState:
    ...
    def set_data_train(self, X_train, y_train,train_indices, train_aux_vars=None):
