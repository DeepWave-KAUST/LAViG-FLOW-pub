...
#
# Description: fno/train_FNO_dp.py
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train FNO pressure buildup model")
...
def main():
    ...
    train_a = torch.load(f"{data_dir}/dP_data/training_dataset_pressure_buildup/dP_train_inputs.pt")
    print(f"loaded train_a in {time.time() - _load_start:.2f}s", flush=True)
    print("loading train_u...", flush=True)
    _load_start = time.time()
    train_u = torch.load(f"{data_dir}/dP_data/training_dataset_pressure_buildup/dP_train_outputs.pt")
    ...
    model = FNO3d(mode1, mode2, mode3, width).to(device)
    ...
    scheduler_step = int(cfg.get("scheduler_step", 4))
    scheduler_gamma = float(cfg.get("scheduler_gamma", 0.85))
    ...
    run_name = f"fno_dp_{time.strftime('%Y%m%d-%H%M%S')}"
    run_config = {
        "model": "fno",
        "target": "dP",
    ...
    for ep in range(1, epochs + 1):
        ...
        if accelerator.is_main_process and ep % save_every == 0:
            path = f"{save_dir}/dP_FNO_{ep}ep_{width}width_{mode1}m1_{mode2}m2_{train_a.shape[0]}train_{lr_:.2e}lr"
