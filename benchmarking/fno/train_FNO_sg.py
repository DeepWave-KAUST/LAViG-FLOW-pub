...
#
# Description: fno/train_FNO_sg.py
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train FNO gas saturation model")
...
def main():
    ...
    run_name = f"fno_sg_{time.strftime('%Y%m%d-%H%M%S')}"
    run_config = {
        "model": "fno",
        "target": "sg",
    ...
    for ep in range(1, epochs + 1):
        ...
        if accelerator.is_main_process and ep % save_every == 0:
            path = f"{save_dir}/SG_FNO_{ep}ep_{width}width_{mode1}m1_{mode2}m2_{train_a.shape[0]}train_{lr_:.2e}lr"
