...
class QuadrupleCartesianProd(Data):
    ...
    def __init__(self, X_train, y_train, X_test, y_test):
        ...
        self.train_timestep_sampler = BatchSampler(len(X_train[2]), shuffle=True)
    ...
    def train_next_batch(self, batch_size=None, timestep_batch_size=None, training_time_size=None):
        ...
        if x_loc.ndim != 2:
            raise ValueError(
                "Expected train_x[2] to have shape (training_time_size, dim). "
                f"Got shape {x_loc.shape}."
            )
        full_time_size = x_loc.shape[0]
        if training_time_size is None:
            training_time_size = full_time_size
        elif training_time_size != full_time_size:
            raise ValueError(
                "training_time_size does not match train_x[2]. "
                f"training_time_size={training_time_size}, train_x[2].shape[0]={full_time_size}."
            )
        size = self.train_y.shape[0]
        return (
            self.train_x[0][indices],
            self.train_x[1][indices],
            x_loc[indices_timestep],
