...
#
# Description: Fourier_MIONet/train_Fourier_MIONet_sg.py
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train Fourier-MIONet gas saturation model")
...
def main():
    ...
    run_name = f"fourier_mionet_sg_{time.strftime('%Y%m%d-%H%M%S')}"
    run_config = {
    ...
    checkpoint_prefix = os.path.join(save_dir, "Fourier_MIONet_SG")
    ...
    model.train(
        ...
        display_every=int(cfg.get("display_every", 3375)),
        init_test=True,
        callbacks=callbacks,
    )
