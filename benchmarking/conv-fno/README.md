...
This folder provides a **conv-FNO** baseline.
...
## Files
- `conv_fno.py`: conv-FNO model (6 Fourier layers, last 3 add Conv3D path).
- `lploss.py`: relative Lp loss.
- `train_ConvFNO_sg.py`: train SG model from YAML config.
- `train_ConvFNO_dp.py`: train dP model from YAML config.
- `train_sg.yaml`: default config for SG training.
- `train_dp.yaml`: default config for dP training.

## Train
```bash
python train_ConvFNO_sg.py --config train_sg.yaml
python train_ConvFNO_dp.py --config train_dp.yaml
```
